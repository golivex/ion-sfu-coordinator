...
import (
	...
	"github.com/manishiitg/ion-sfu-cluster-load-tool/client"
	"github.com/manishiitg/ion-sfu-cluster-load-tool/client/gst"
	...
	for i := 0; i < total; i++ {
		...
		case "pubsub":
			...
			if !strings.Contains(file, ".webm") {
				...
				go producer.Start()
				c.Publish(producer.VideoTrack())
				c.Publish(producer.AudioTrack())
